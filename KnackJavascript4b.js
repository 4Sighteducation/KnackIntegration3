...
function standardizeCards(cards) {
  ...
  return cards.map(card => {
    // If it's already a standard card, fix type if needed then return it
    if (card.createdAt && card.updatedAt && card.examBoard !== undefined) {
      // Check if this is a multiple choice card by answer pattern
      if (card.answer && typeof card.answer === 'string' && 
          card.answer.match(/Correct Answer:\s*[a-e]\)/i)) {
        // Force proper typing for multiple choice
    ...
    // Check if this is a multiple choice card by answer pattern
    const isMultipleChoice = card.answer && 
    ...
    return {
      ...
      // Card type - use multiple_choice if detected from answer pattern
      type: isMultipleChoice ? 'multiple_choice' : (card.type || 'short_answer'),
      questionType: isMultipleChoice ? 'multiple_choice' : (card.questionType || 'short_answer'),
