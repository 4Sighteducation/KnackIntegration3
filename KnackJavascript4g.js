...
function createTopicShellsFromLists(topicLists, recordId) {
  try {
    ...
    // Track the unique topics we're processing for logging
    ...
    topicLists.forEach(list => {
      ...
      list.topics.forEach(topic => {
        ...
        // Track this unique topic for logging
    ...
    console.log(`Flashcard app [${new Date().toISOString()}]: Created ${topicShells.length} topic shells for ${uniqueTopics.size} unique topics`);
    
    // Sample the first shell for logging
    const sampleShell = topicShells[0];
    console.log(`Flashcard app [${new Date().toISOString()}]: Sample topic shell:`, {
      id: sampleShell.id,
      name: sampleShell.name,
      topic: sampleShell.topic,
      subject: sampleShell.subject
    });
          ...
          function attemptShellSave() {
            $.ajax({
              ...
              success: function(response) {
                ...
                // Notify the React app that card bank data has been updated
                // This helps the UI refresh automatically without requiring a page reload
