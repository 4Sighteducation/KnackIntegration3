...
function createTopicShellsFromLists(topicLists, recordId) {
  try {
    ...
    topicLists.forEach(list => {
      if (!list.topics || !Array.isArray(list.topics)) return;
      
      const subject = list.subject || 'General';
      const examBoard = list.examBoard || '';
      const examType = list.examType || '';
      
      // Process each topic in this list
      list.topics.forEach(topic => {
        ...
        // Create a topic shell
        const topicShell = {
          id: topicId,
          type: 'topic',
          name: topicName,
          topic: topicName, // Include both name AND topic properties
          subject: subject,
          examBoard: examBoard,
          examType: examType,
    ...
    console.log(`Flashcard app [${new Date().toISOString()}]: Created ${topicShells.length} topic shells`);
    
    // Get the current card bank data
    $.ajax({
      url: KNACK_API_URL + '/objects/' + FLASHCARD_OBJECT + '/records/' + recordId,
      type: 'GET',
      headers: {
      ...
      success: function(existingData) {
        try {
          ...
          // Add new topic shells, avoiding duplicates
          const finalTopicShells = [];
          
          topicShells.forEach(shell => {
            if (existingTopicMap.has(shell.id)) {
              // Keep the existing shell
              finalTopicShells.push(existingTopicMap.get(shell.id));
            } else {
              // Add the new shell
              finalTopicShells.push(shell);
            }
          });
          ...
          $.ajax({
            ...
            success: function(response) {
              console.log(`Flashcard app [${new Date().toISOString()}]: Successfully added ${finalTopicShells.length} topic shells`);
            },
            error: function(error) {
          ...
          console.error(`Flashcard app [${new Date().toISOString()}]: Error processing data for topic shell creation:`, error);
        }
      },
      error: function(error) {
        console.error(`Flashcard app [${new Date().toISOString()}]: Error retrieving data for topic shell creation:`, error);
      }
    });
  } catch (error) {
      ...
      success: function(existingData) {
        ...
        if ((!cleanTopicLists || cleanTopicLists.length === 0) && existingData[FIELD_MAPPING.topicLists]) {
          ...
          if (Array.isArray(existingTopicLists) && existingTopicLists.length > 0) {
            ...
            updateData[FIELD_MAPPING.topicLists] = JSON.stringify(existingTopicLists);
          } else {
    ...
    $.ajax({
      ...
      success: function(existingData) {
        try {
          ...
          // Process the new cards to match with topic shells
          const processedNewCards = standardizedCards.map(card => {
            // Try to find a topic shell that matches this card's topic
            const matchingShell = topicShells.find(shell => 
              shell.topic === card.topic && 
            ...
            return card; // No matching shell, keep as is
          });
          ...
          const newBox1Entries = processedNewCards.map(card => ({
            cardId: card.id,
            lastReviewed: now,
