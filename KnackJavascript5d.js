...
(function() {
  ...
  // Safely decode URI components, handling potential errors
  ...
  // Safely parse JSON, handling potential errors and empty strings
  function safeParseJSON(jsonString) {
    if (!jsonString || typeof jsonString !== 'string') {
      return null; // Return null for empty or non-string input
    }
    try {
      // Attempt to decode if it looks URL-encoded
      ...
      // Attempt to fix common issues like trailing commas or improperly escaped quotes
      try {
          // Remove trailing commas (simple approach)
          let fixedString = jsonString.replace(/,\s*([\]}])/g, '$1');
          // Replace improperly escaped quotes (basic attempt)
          fixedString = fixedString.replace(/\\"/g, '"');
          return JSON.parse(fixedString);
      } catch (fixError) {
          console.error("Error parsing JSON even after attempting fixes:", fixError);
          return null; // Return null if parsing fails even after fixes
      }
    }
  }
  ...
  function retryApiCall(apiCall, maxRetries = 3, delay = 1000) {
    return new Promise((resolve, reject) => {
      ...
      attempt(1); // Start the first attempt
    });
  }
  ...
  function migrateTypeToQuestionType(data) {
      if (!Array.isArray(data)) {
          console.warn("migrateTypeToQuestionType: Input is not an array", data);
      ...
      return data.map(item => {
          if (item && typeof item === 'object' && 'type' in item && !('questionType' in item)) {
              ...
              delete newItem.type; // Remove the old 'type' field
              return newItem;
          }
  ...
  // Basic check for Knack ID format (24 hex characters)
  ...
  function cleanHtmlFromId(idString) {
      if (typeof idString !== 'string') return null; // Return null if not a string

      // Use DOMParser to safely handle HTML
      const parser = new DOMParser();
      const doc = parser.parseFromString(idString, 'text/html');

      // Extract text content which should be the ID if wrapped in HTML
      let potentialId = doc.body.textContent || "";
      potentialId = potentialId.trim(); // Trim whitespace

      // Validate if it looks like a Knack ID
      if (isValidKnackId(potentialId)) {
          return potentialId;
      }

      // If it wasn't wrapped in HTML but is a valid ID, return it
      if (isValidKnackId(idString.trim())) {
          return idString.trim();
      }

      // If it's an object ID from Knack's format (e.g., [{id: 'xxx'}])
      try {
  ...
  // Function to extract a valid Knack record ID from various possible value formats
  function extractValidRecordId(value) {
      ...
      // 2. Array with an object containing 'id' (common Knack connection format)
      if (Array.isArray(value) && value.length > 0 && value[0] && isValidKnackId(value[0].id)) {
          return value[0].id;
      }

      // 3. Object with 'id' property
      if (typeof value === 'object' && value !== null && isValidKnackId(value.id)) {
          return value.id;
      }

      // 4. String containing HTML (attempt to clean)
      if (typeof value === 'string') {
          const cleanedId = cleanHtmlFromId(value);
          if (cleanedId) return cleanedId;
      }

      // 5. Check if it's a raw user object with an 'id'
      if (typeof value === 'object' && value !== null && value.id && isValidKnackId(value.id)) {
         return value.id;
       }
  ...
  // Sanitize field values before sending to Knack API
  function sanitizeField(value) {
    if (value === null || typeof value === 'undefined') {
      return ''; // Return empty string for null or undefined
    }
    // Convert non-string values to string, trim whitespace
    let stringValue = String(value).trim();
    // Basic HTML tag removal (consider a more robust library if complex HTML is expected)
    stringValue = stringValue.replace(/<[^>]*>?/gm, '');
    // Optional: Limit length if necessary (e.g., Knack field limits)
    // const maxLength = 255;
  ...
  function debugLog(title, data) {
      // Check if debug mode is enabled (e.g., via a global variable or query parameter)
  ...
  function initializeFlashcardApp(containerElement) {
    ...
    // --- Use the passed element as the container --- 
    var container = containerElement; // Use the passed element directly
    ...
    // Check if containerElement is valid (pre-checked in listener, but good practice)
    if (!container) {
        console.error(`Flashcard app: Invalid container element passed. Cannot create iframe.`);
        ...
        if (event.data && event.data.type) {
          switch(event.data.type) {
            ...
            case 'PERSISTENCE_SERVICES_READY':
              console.log("Flashcard app: Received PERSISTENCE_SERVICES_READY signal from React app.");
              // Attempt to load services from the iframe window object
              if (loadPersistenceServices()) {
                  console.log("Flashcard app: Successfully loaded persistence services from iframe window after signal.");
                  // Acknowledge back to React App (optional, React app might not need this)
                  if (iframe && iframe.contentWindow) {
                    iframe.contentWindow.postMessage({
                  ...
                  console.error("Flashcard app: Failed to load persistence services from iframe window even after signal.");
              }
              // We no longer try to read services from event.data
              break;
          }
        }
      });
  ...
  $(document).on('knack-view-render.view_3005', function(event, view) {
    console.log("Knack view view_3005 rendered. Finding element by CLASS...");
    
    // --- Find the element manually using CLASS selector --- 
    var viewElementContainer = document.querySelector('.view_3005'); // Look for the class

    if (viewElementContainer) {
        console.log("Found element with class .view_3005. Proceeding to initialize flashcard app.");
        // Pass the found container DOM element directly to the initialization function
        initializeFlashcardApp(viewElementContainer);
    } else {
        // This error means the element with class view_3005 wasn't in the DOM after its render event fired.
        console.error("Critical Error: Could not find element with CLASS .view_3005 in the DOM immediately after render event!");
        var loadingDiv = document.getElementById('loading-indicator');
        if (loadingDiv) loadingDiv.textContent = 'Error: App container could not be found after loading.'; 
    }
  });
  // --- END Knack listener ---
