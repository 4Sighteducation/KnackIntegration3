...
(function() {
  ...
  function initializeFlashcardApp() {
    ...
    // --- Added: Define iframe and loadingDiv --- 
    var iframe = document.getElementById('flashcard-app-iframe'); 
    var loadingDiv = document.getElementById('loading-indicator'); // Assuming this ID
    var userToken = Knack.getUserToken(); // Get token early
    var appId = Knack.app.id; // Get app ID early
    var currentUser = Knack.getUserAttributes(); // Get user attributes early
    // --- End Added --- 
    ...
    if (Knack && userToken) { // Use the fetched token
      ...
      // --- Added: Check if iframe exists before adding listener ---
      if (!iframe) {
        console.error("Flashcard app: Could not find iframe with ID 'flashcard-app-iframe'. Cannot initialize message listener.");
        // Optionally hide loading indicator if it exists
        if (loadingDiv) {
            loadingDiv.style.display = 'none';
            // Display an error message to the user?
            // loadingDiv.innerHTML = 'Error: Flashcard application could not be loaded.';
        }
        return; // Stop initialization if iframe is missing
      }
      // --- End Added Check ---
      ...
      window.addEventListener('message', function(event) {
        ...
        // --- Added: Check if iframe and contentWindow exist before accessing ---
        ...
        if (event.data && event.data.type) {
          switch(event.data.type) {
            case 'APP_READY':
              ...
              loadFlashcardUserData(user.id, function(userData) {
                ...
                const userDataToSend = {
                  ...
                  userData: userData || {},
                  // Add connection field IDs - Use safely extracted IDs
                  emailId: extractValidRecordId(currentUser.id), // User's own ID for email connection
                  schoolId: extractValidRecordId(currentUser.field_122), // Assuming field_122 is school
                  tutorId: extractValidRecordId(currentUser.field_xxx), // Replace field_xxx with actual tutor field ID
                  roleId: extractValidRecordId(currentUser.field_73) // Assuming field_73 is role
                };
            ...
            case 'AUTH_CONFIRMED':
              ...
              // --- Added: Check if elements exist before manipulating style ---
            ...
            case 'ADD_TO_BANK':
              ...
              handleAddToBankPromise(event.data.data)
                .then((success) => {
                  ...
                  setTimeout(() => {
                    // --- Added: Check iframe and contentWindow --- 
  ...
  if (window.parent) {
    // --- Added: Check if services are actually defined before sending ---
    if (typeof unifiedPersistenceManager !== 'undefined' && 
        typeof topicShellManager !== 'undefined' && 
        typeof metadataManager !== 'undefined' && 
        typeof colorManager !== 'undefined' && 
        typeof dataOperationQueue !== 'undefined') {
        ...
        console.warn("KnackJavascript: One or more persistence services are not defined. Cannot send PERSISTENCE_SERVICES_READY message.");
    }
    // --- End Added Check ---
  }
  ...
  // Make sure to include all the required functions from KnackJavascript4q.js
  ...
  $(document).ready(function() {
    ...
    console.log("Document ready - initializing flashcard app");
