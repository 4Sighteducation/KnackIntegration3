...
(function() {
  ...
  $(document).on('knack-view-render.view_3005', function(event, view) {
    console.log("Knack view view_3005 rendered. Event object:", event);
    console.log("View object received:", view);
    // --- Added Debugging ---
    console.log("Inspecting view.el:", view.el);
    if (view && view.el) {
        console.log("view.el seems valid, proceeding to initialize flashcard app.");
        // Pass the specific view object to the initialization function
        initializeFlashcardApp(view);
    } else {
        console.error("Critical Error: view object or view.el is missing/invalid immediately after render event!", { view_object: view });
        // Optionally, try to find the view manually as a fallback (less reliable)
        // var fallbackViewEl = document.getElementById('view_3005');
        // if (fallbackViewEl) {
        //    console.warn("Attempting initialization with fallback element found by ID.");
        //    initializeFlashcardApp({ el: fallbackViewEl, key: 'view_3005' }); // Construct a minimal view-like object
        // } else {
        //    console.error("Fallback failed: Could not find element #view_3005 either.");
        // }
    }
    // --- End Debugging ---
  });
