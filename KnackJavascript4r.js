...
function createTopicShellsFromLists(topicLists, recordId) {
  try {
    ...
    if (loadPersistenceServices() && topicShellManager) {
      ...
      unifiedPersistenceManager.saveTopics(createdShells)
        ...
        .catch(error => {
          ...
          unifiedPersistenceManager.rollbackTransaction()
            ...
            .finally(() => {
              // Use direct API call instead of falling back to legacy method
      ...
      // Use direct API call instead of falling back to legacy method
      console.log(`Flashcard app [${new Date().toISOString()}]: No persistence manager, using direct API approach`);
      createTopicShellsDirectAPI(topicLists, recordId);
    }
  } catch (error) {
    console.error(`Flashcard app [${new Date().toISOString()}]: Error in createTopicShellsFromLists:`, error);
    
    // Use direct API call for error recovery
    createTopicShellsDirectAPI(topicLists, recordId);
  }
}
    ...
    Promise.all(promises)
      ...
      .catch(error => {
        ...
        unifiedPersistenceManager.rollbackTransaction()
          ...
          .finally(() => {
            // Use direct API instead of legacy method
            console.log("Flashcard app: Using direct API for saving data");
            saveUserDataDirectAPI(userId, data, callback);
          });
      });
  } else {
    // Use direct API instead of legacy method
    console.log("Flashcard app: No persistence manager available, using direct API for saving");
    saveUserDataDirectAPI(userId, data, callback);
  }
}

// Direct API implementation for saving user data
function saveUserDataDirectAPI(userId, data, callback) {
  console.log("Flashcard app: Saving with direct API for:", userId);
...
function handleAddToBank(data, callback) {
  try {
    ...
    if (loadPersistenceServices()) {
      ...
      // Use direct API instead of legacy method
      console.log("Flashcard app: No persistence manager available, using direct API");
      addToBankDirectAPI(data, callback);
    }
  } catch (error) {
